...
public class SimpleSpecification<T> implements Specification<T> {
    ...
    private Predicate generatePredicate(Root<T> root,CriteriaBuilder criteriaBuilder, SpecificationOperator op) {
        ...
        * 根据不同的操作符返回特定的查询*/
        ...
        } else if(SpecificationOperator.Operator.datele.name().equalsIgnoreCase(op.getOper())) {
            return criteriaBuilder.lessThanOrEqualTo(root.get(op.getKey()).as(Date.class),(Date)op.getValue());
        } else if(SpecificationOperator.Operator.datege.name().equalsIgnoreCase(op.getOper())) {
            return criteriaBuilder.greaterThanOrEqualTo(root.get(op.getKey()).as(Date.class), (Date) op.getValue());
        }
        return null;
    }
